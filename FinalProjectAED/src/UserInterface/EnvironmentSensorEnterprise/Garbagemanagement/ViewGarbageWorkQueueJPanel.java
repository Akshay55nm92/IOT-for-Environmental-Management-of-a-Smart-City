 ...
 * @author Akshay
 */
public class ViewGarbageWorkQueueJPanel extends javax.swing.JPanel {
     ...
     */
    public ViewGarbageWorkQueueJPanel(JPanel userProcessContainer, UserAccount userAccount, OrganizationDirectory directory) {
        initComponents();
        ...
        for(Organization organization : directory.getOrganizationList())
        {
            if(organization instanceof EnvironmentSensorOrganization)
            {
                for (WorkRequest wr : organization.getWorkQueue().getWorkRequestList()) {
                    ...
                    if(!garbage_wrs.getStatus().equalsIgnoreCase("Resolved"))
                    {
                        Object[] row = new Object[7];
                        row[0] = garbage_wrs;
                        row[1] = garbage_wrs.getSender();
    ...
    private void SowSensorReadingButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SowSensorReadingButtonActionPerformed
        ...
        {
            GarbageManagementWorkRequest garbageManagementWorkRequest = (GarbageManagementWorkRequest) ResolvedWorkQueuejTable.getValueAt(row, 0);
            GarbageSensor garbageSensor = garbageManagementWorkRequest.getGarbagesensor();

            TimeSeries s1 = new TimeSeries("Sensor Rating");
            TimeSeriesCollection dataset = new TimeSeriesCollection();
            final JFreeChart chart = ChartFactory.createTimeSeriesChart("Garbage Container value vs Date and Time",
                "Timestamp",
                "Garbage Container value", dataset,
                true,
                true,
                false);
            ...
            for (Map.Entry<Date, Float> entry : garbageSensor.getSensorValues().entrySet()) {
